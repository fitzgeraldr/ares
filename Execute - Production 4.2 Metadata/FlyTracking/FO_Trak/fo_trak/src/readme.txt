    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_fo_trak.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: fo_trak.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * fo_trak.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * fo_trak.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * fo_trak (application created by MATLAB Compiler)
        * run_fo_trak.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * fo_trak.so (shared library, file extension varies by 
          platform)
        * fo_trak.h (library header file)
    ...
    Windows:
        * fo_trak.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * fo_trak.xml (documentation files)
        * fo_trak.pdb (program database file - if DEBUG option 
          selected)
        * fo_trak.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * fo_trak.jar (Java class files and compiled M-code)
    ...
    Windows:
        * fo_trak.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * fo_trak_1_0.dll (COM component)
        * fo_trak.xla (optionally created and placed in
          /groups/flyprojects/home/olympiad/FO_Trak/fo_trak/src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
