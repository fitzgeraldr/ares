...
% AL: Note that post-processing has modified the obj_info in indeterminate
% (to me) ways. So for example at this stage the following line will
% assert:
% obj_info_validate(obj_info,obj_info.last_frame); 
...
if start_frame~=1
    % AL: seems dangerous as frame_num will no longer equal the true number
    % of frames in the sequence (eg global numFrames), with the consequence
    % that analysis_info fields below will be initialized to the wrong
    % size...
    warning('analysis_olympiad:start_frameNotOne',...
        'start_frame is %d rather than 1.',start_frame);
end

% AL: computation of trackedFrames duplicated in main()
...
analysis_info.moving_num_left = zeros(1,frame_num);
analysis_info.moving_num_right = zeros(1,frame_num);
...
for f=1:frame_num-1
    ...
    %%% moving number/percentage, right and left
    tf_moving_left = distance>move_thresh & dx<0;
    tf_moving_right = distance>move_thresh & dx>0;
    analysis_info.moving_num_left(f) = nnz(tf_moving_left);
    analysis_info.moving_num_right(f) = nnz(tf_moving_right);    
    
    tmpMovingButNotLeftOrRight = analysis_info.moving_num(f) - ...
        (analysis_info.moving_num_left(f) + analysis_info.moving_num_right(f));    
    assert(tmpMovingButNotLeftOrRight>=0,'moving_num_left+moving_num_right exceeds total moving_num.');
    assert(tmpMovingButNotLeftOrRight==nnz(distance>move_thresh & ~(dx>0 | dx<0)),...
        'Error computing moving_num_left/right.');
