...
% 'trxname': name of mat file containing trajectories, or struct of trajectories
...
% 'compression': compressor to use when outputting (pc only, will be set to 'none' for Linux).
% 'flipud': flip movie vertically?
...
[moviename,trxname,aviname,colors,zoomflies,nzoomr,nzoomc,boxradius,taillength,fps,maxnframes,firstframe,compression,flip_movie_ud] = ...
  myparse(varargin,'moviename','','trxname','','aviname','','colors',[],'zoomflies',[],'nzoomr',nan,'nzoomc',nan,...
  'boxradius',nan,'taillength',nan,'fps',nan,'maxnframes',nan,'firstframe',nan,'compression','','flipud',nan);
...
if isnan( flip_movie_ud )
   prompts{end + 1} = 'Flip movie vertically?';
   defaultanswers{end + 1} = num2str( 0 );
end
  ...
  while true,
    ...
    for i = 1:length(answers),
      ...
      if isempty(answers{i}) || answers{i} < 0,
        uiwait(msgbox('All answers must be non-negative numbers'));
        failed = true;
        break;
      end
      switch prompts{i},
        ...
        case 'First frame to output',
         ...
         case 'Flip movie vertically?',
            flip_movie_ud = max( 0, ceil( answers{i} ) );
            answers{i} = flip_movie_ud;
      end
