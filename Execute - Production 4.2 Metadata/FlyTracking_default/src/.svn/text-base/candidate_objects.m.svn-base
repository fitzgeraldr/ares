...
I_fg_bw = call_with_diag([],'immultiply',I_fg_bottom,I_occlud); % only use bottom difference for Olypiad videos
...
%I_fg_bw = ordfilt2(I_fg_bw,5,ones(3,3));
%I_fg_bw = ordfilt2(I_fg_bw,5,ones(3,3));
I_fg_bw = call_with_diag([],'ordfilt2',I_fg_bw,5,ones(3,3));
I_fg_bw = call_with_diag([],'ordfilt2',I_fg_bw,5,ones(3,3));
I_fg_bw = call_with_diag([],'ordfilt2',I_fg_bw,5,ones(3,3));
...
I_label = call_with_diag([],'bwlabel',I_fg_bw);

%%%% estimate probability of each object (max probability)
tmp_immultiply = call_with_diag([],'immultiply',I_fg,I_fg_bw);
I_prob = double(tmp_immultiply)/256;
