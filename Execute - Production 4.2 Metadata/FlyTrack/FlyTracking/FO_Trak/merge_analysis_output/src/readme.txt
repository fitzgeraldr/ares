    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_merge_analysis_output.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: merge_analysis_output.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * merge_analysis_output.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * merge_analysis_output.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * merge_analysis_output (application created by MATLAB Compiler)
        * run_merge_analysis_output.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * merge_analysis_output.so (shared library, file extension varies by 
          platform)
        * merge_analysis_output.h (library header file)
    ...
    Windows:
        * merge_analysis_output.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * merge_analysis_output.xml (documentation files)
        * merge_analysis_output.pdb (program database file - if DEBUG option 
          selected)
        * merge_analysis_output.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * merge_analysis_output.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * merge_analysis_output_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * merge_analysis_output.jar (Java class files and compiled M-code)
    ...
    Windows:
        * merge_analysis_output.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * merge_analysis_output_1_0.dll (COM component)
        * merge_analysis_output.xla (optionally created and placed in
          /groups/flyprojects/home/olympiad/FO_Trak/merge_analysis_output/src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
