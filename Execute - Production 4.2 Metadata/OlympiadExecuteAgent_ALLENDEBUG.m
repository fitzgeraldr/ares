...
function OlympiadExecuteAgent(obj,~,fig)
        ...
        case 1
            ...
            switch expstate.tr
                case 0
                    ...
                    fclose(gui.MCU);
                    gui.MCU.bytesavailablefcn={@exec_BAF,gui.fig,obj};
    ...
    switch get(gui.splitTubes,'value')
        case 0 %full frame
            for i=1:length(seq.vidinds)
                ...
                fprintf(1,'ALLENTEST: opening avi, fps=%.3g\n',...
                          1000/action(seq.vidinds(i)).command(2));
                ff = ('c:\documents and settings\olympiad\Desktop\ALLENTEST.avi');
                %ff=fullfile(seq.dir,fname);
            ...
            fprintf('ALLENALLEN INITTED WRIETFULLAVI\n');
            WriteFullAVI();
...
if nargin==0
    ctr=0;
    fprintf(1,'WriteFullAvi init\n');
    return;
else
    ctr = ctr+1;
    fprintf(1,'WriteFullAvi %d\n',ctr);
end
...
fprintf(1,'ALLEN: HARDCODED COMMAND FIRST TWO ROWS [3 350 0 0 0] [3 0 0 0 0]\n');
command(1,:) = [3 415 0 0 0];
command(2,:) = [3 0 0 0 0];
%Write Commands
sendcommand=[];
...
function exec_BAF(obj,~,fig,agent)
