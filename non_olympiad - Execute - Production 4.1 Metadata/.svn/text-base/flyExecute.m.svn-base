...
switch get(gui.ExperimentGo,'value')
...
%         end
        ...
        set(agent,'timerfcn',{@OlympiadExecuteAgent,gui.fig},'TasksToExecute',inf)
        expstate.dbstate = -2;
...
global gui; %Code modified in V3.0
...
gui.SetDir=uicontrol(gui.MCUPanel,'style','pushbutton','units','normalized','position',[.05 .7 .4 .15],'string','Set Directory',...
    'userdata','E:\','callback',{@SetExpDir,gui.fig});
...
gui.ExperimentGo=uicontrol(gui.MCUPanel,'style','toggle','units','normalized','position',[.55 .22 .4 .24],'string','Run Experiment',...
    'backgroundcolor',[.9 .2 .2],'fontweight','bold','enable','off','callback',{@ExecuteExperiment,gui.fig});
%Status Text
...
%_________________________________________________________________________________________
%GUI handling for opening camera config properties
